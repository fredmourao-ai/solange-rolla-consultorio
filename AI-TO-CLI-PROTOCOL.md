...
8. Nao pedir confirmacao intermediaria quando a intencao do usuario ja estiver clara e existir alternativa segura que nao aumente escopo ou risco. Esta dispensa nunca se aplica a acao irreversivel ou de alto raio de impacto (merge/push em branch compartilhada, force-push, deploy/promocao para producao ou homologacao, operacao destrutiva de dados ou infraestrutura): essas sempre exigem confirmacao humana explicita antes de executar, mesmo que a intencao geral da tarefa ja seja conhecida. Para auto-merge deste repositorio, a confirmacao deve ser persistida no PR como comentario exato `HUMAN_MERGE_APPROVED_SHA:<HEAD_SHA>` pelo owner; qualquer alteracao do HEAD exige nova confirmacao. Handoff automatico para staging exige aprovacao independente no mesmo PR como `HUMAN_STAGING_APPROVED_HEAD:<HEAD_SHA>`; ausencia desse marcador mantem a promocao bloqueada ate autorizacao explicita.
