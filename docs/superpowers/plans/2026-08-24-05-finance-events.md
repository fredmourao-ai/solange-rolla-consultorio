...
**Architecture:** `receivables` e `payables` são domínios financeiros separados; `events` referencia Pessoa e solicita recebíveis por contrato público. Valores monetários são `bigint` em centavos. Toda baixa/ajuste é auditável e idempotente.
...
- Ajuste, desconto, isenção e estorno exigem motivo e ator.
...
- Produces status `open|partial|paid|overdue|voided|refunded`.
...
Criar `receivables`, `receivable_adjustments`, constraints de valores não negativos, unique para source/idempotency e timestamps. `balance_cents` deve ser derivado de lançamentos ou mantido por transação com testes de consistência; nunca editado manualmente pela UI.

- [ ] **Step 4: Política de consulta**

Ao criar consulta cobrável, criar recebível previsto uma única vez. `cancelled_in_time` pode voidar recebível ainda sem pagamento; `cancelled_late`/`no_show` mantém ou cria cobrança conforme snapshot da política. Isenção manual gera adjustment `waiver` com motivo.

- [ ] **Step 5: Testes**

Cobrir consulta realizada, cancelada dentro do prazo, fora do prazo, falta e isenção. Confirmar que appointment status não é alterado pelo módulo financeiro.
...
### Task 2: Pagamentos, múltiplas formas e estornos
...
Criar `payments`, `payment_refunds`; `idempotency_key unique`; amount > 0; `paid_at timestamptz`; audit actor. Estorno nunca deleta pagamento original.

- [ ] **Step 4: UI de baixa**

Dialog mostra valor previsto, saldo, valor recebido, data/hora e forma. Permite múltiplas baixas sequenciais e exibe histórico.

- [ ] **Step 5: Testar concorrência**

Dois requests simultâneos tentando quitar o mesmo saldo não podem exceder o saldo; usar lock/transação no repositório.

- [ ] **Step 6: Commit**

```bash
git add supabase src/modules/receivables
 git commit -m "feat: add auditable payments"
```
...
Regra dia 10, iniciada em setembro/2026, gera exatamente uma obrigação por competência, mesmo com job executado repetidamente.
...
**Interfaces:**
- Produces `CashflowSnapshot` com recebido, a receber, vencido, pago, a pagar, realizado e projetado.

- [ ] **Step 1: Criar fixture sintética**

Receitas: 1285000 recebidos + 240000 abertos; despesas: 415000 pagas + 123000 futuras. Teste deve validar totais em centavos, sem arredondamento binário.

- [ ] **Step 2: Implementar read model**

Resultado realizado = pagamentos recebidos - despesas pagas no período. Projetado = realizado + recebíveis previstos elegíveis - payables a vencer, sempre com labels de que projeção não é contabilidade oficial.

- [ ] **Step 3: Separar competência e caixa**

Queries devem permitir filtro por `paid_at` (caixa) e por competência/origem quando necessário; não misturar silenciosamente.

- [ ] **Step 4: Testar timezone de fechamento mensal**

Pagamento às 00:30 UTC deve ser classificado pelo dia/mês local em `America/Sao_Paulo` quando relatório for por data de negócio.
...
Calcular vagas, inscritos, receita potencial, recebido, a receber, despesas e resultado do evento a partir das fontes financeiras reais.

- [ ] **Step 3: Cancelamento/reembolso**

Cancelamento de inscrição não apaga pagamento; se houver devolução, usar refund. Regras de cancelamento de evento ficam separadas das regras de consulta até definição explícita.
