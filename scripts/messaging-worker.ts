...
function buildWhatsAppProvider(live: boolean): MessagingProvider {
  ...
  return createMetaWhatsAppProvider(createFetchWhatsAppTransport(process.env.WHATSAPP_API_VERSION ?? 'v23.0'))
}
...
async function main() {
  ...
  async function sendOutboundMessage(
    ...
    if (activeVersion === null) {
      log('messaging_template_not_active', {
        messageId,
  ...
  const confirmationInterval = positiveInt(
    process.env.APPOINTMENT_CONFIRMATION_SCHEDULER_INTERVAL_MS,
    120_000,
    30_000,
    3_600_000,
  )
  const birthdayInterval = positiveInt(
    process.env.BIRTHDAY_SCHEDULER_INTERVAL_MS,
    3_600_000,
    300_000,
    86_400_000,
  )
  ...
  async function enqueueIfNew(message: Parameters<typeof enqueueMessage>[0]): Promise<boolean> {
    if (await messages.findByIdempotencyKey(message.idempotencyKey)) return false
    await enqueueMessage(message, messages)
    return true
  }

  async function scheduleBirthdayGreetingsIfDue(now: Date): Promise<void> {
    if (now.getTime() - lastBirthday < birthdayInterval) return
    lastBirthday = now.getTime()
    const scheduled = await scheduleBirthdayGreetings({ now, enqueue: enqueueIfNew })
    if (scheduled > 0) log('birthday_greetings_scheduled', { scheduled })
  }

  async function scheduleAppointmentConfirmationsIfDue(now: Date): Promise<void> {
    if (now.getTime() - lastConfirmation < confirmationInterval) return
    lastConfirmation = now.getTime()
  ...
  async function scheduleAutomations(): Promise<void> {
    const now = new Date()
    await scheduleBirthdayGreetingsIfDue(now)
    await scheduleAppointmentConfirmationsIfDue(now)
  }
  ...
  const drain = async () => {
    try {
      ...
      log('messaging_automation_scheduling_failed', {
  ...
  log('messaging_worker_started', {
    batchSize,
    pollMs,
    birthdayInterval,
    confirmationInterval,
    whatsappLive: env.WHATSAPP_LIVE_ENABLED,
    emailLive: env.EMAIL_LIVE_ENABLED,
  })
