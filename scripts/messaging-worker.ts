...
function buildEmailProvider(live: boolean): MessagingProvider {
  ...
  const transport = createSmtpEmailTransport({ host: requireEnv('EMAIL_SMTP_HOST'), port, secure: port === 465, user: requireEnv('EMAIL_SMTP_USER'), password: requireEnv('EMAIL_SMTP_PASSWORD') })
...
async function main() {
  ...
  async function sendOutboundMessage(messageId: string, attemptNumber: number, retry: (delaySeconds: number) => Promise<void>): Promise<'sent' | 'retry' | 'failed'> {
    const message = await messages.findById(messageId); if (!message) { log('messaging_message_not_found', { messageId }); return 'failed' }; if (message.status === 'sent') return 'sent'
    const activeVersion = await templates.findActiveVersion(message.templateKey, message.channel); if (activeVersion === null) { log('messaging_template_not_active', { messageId, templateKey: message.templateKey, channel: message.channel }); await attempts.markFailed(messageId); return 'failed' }
    const body = await renderVersionedTemplate({ key: message.templateKey, channel: message.channel, version: activeVersion, values: message.payload }, templates)
    return processMessage({ messageId, attemptNumber, message: { channel: message.channel, recipient: message.recipient, body, idempotencyKey: message.idempotencyKey } }, { provider: providerFor(message.channel), attempts, retryQueue: { requeue: retry } })
  }
  const batchSize = positiveInt(process.env.MESSAGING_WORKER_BATCH_SIZE, 10, 1, 50); const pollMs = positiveInt(process.env.MESSAGING_WORKER_POLL_MS, 2000, 250, 60000); const heartbeatPath = process.env.MESSAGING_WORKER_HEALTH_FILE ?? '/tmp/solange-messaging-worker.heartbeat'; const confirmationSchedulingIntervalMs = positiveInt(process.env.APPOINTMENT_CONFIRMATION_SCHEDULER_INTERVAL_MS, 120_000, 30_000, 3_600_000); let lastConfirmationSchedulingRun = 0
  async function scheduleAppointmentConfirmationsIfDue(): Promise<void> {
    const now = new Date(); if (now.getTime() - lastConfirmationSchedulingRun < confirmationSchedulingIntervalMs) return; lastConfirmationSchedulingRun = now.getTime()
    const candidates = await loadConfirmationCandidates(now)
    const scheduled = await scheduleDueAppointmentConfirmations({ now, candidates, alreadyEnqueued: async (idempotencyKey) => (await messages.findByIdempotencyKey(idempotencyKey)) !== null,
      issueLink: async (candidate) => { const { rawToken } = await issueCapability({ purpose: 'appointment_response', subjectType: 'appointment', subjectId: candidate.id, expiresAt: candidate.startsAt, now }, capabilityIssuance); return `${env.APP_URL}/c/${rawToken}?purpose=appointment_response` },
      enqueue: async (message) => { if (await messages.findByIdempotencyKey(message.idempotencyKey)) return false; await enqueueMessage(message, messages); return true },
  ...
  const drain = async () => { try { await scheduleAppointmentConfirmationsIfDue() } catch (error) { log('appointment_confirmation_scheduling_failed', { message: error instanceof Error ? error.message : 'unknown' }) }; const dispatched = await dispatchOutbox(outbox, queue, batchSize); if (dispatched > 0) log('messaging_outbox_dispatched', { dispatched }); return drainMessagingQueueOnce({ queue, process: sendOutboundMessage, batchSize }) }
  const heartbeat = () => writeFile(heartbeatPath, new Date().toISOString(), { encoding: 'utf8', mode: 0o600 })
  log('messaging_worker_started', { batchSize, pollMs, whatsappLive: env.WHATSAPP_LIVE_ENABLED, emailLive: env.EMAIL_LIVE_ENABLED }); await runMessagingWorker({ signal: controller.signal, drain, pollMs, logger: log, heartbeat }); log('messaging_worker_stopped')
}
