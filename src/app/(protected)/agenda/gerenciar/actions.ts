...
export async function createAppointmentAction(formData: FormData) {
  ...
  const repository: AppointmentRepository = { async insert(input) { const { data, error } = await client.rpc('create_appointment_atomic', { p_appointment_id: input.id, p_person_id: input.personId, p_service_id: input.serviceId, p_starts_at: input.startsAt.toISOString(), p_ends_at: input.endsAt.toISOString(), p_policy_version: input.policyVersion, p_cancellation_deadline_at: input.cancellationDeadlineAt, p_business_timezone: 'America/Sao_Paulo', p_cancellation_policy_snapshot: input.cancellationPolicySnapshot as unknown as Database['public']['Functions']['create_appointment_atomic']['Args']['p_cancellation_policy_snapshot'] }); if (error || data !== input.id) throw new Error('AGENDA_CREATE_FAILED'); return { id: input.id, personId: input.personId, serviceId: input.serviceId, startsAt: input.startsAt.toISOString(), endsAt: input.endsAt.toISOString(), status: input.status as Appointment['status'], policyVersion: input.policyVersion, cancellationDeadlineAt: input.cancellationDeadlineAt, cancellationPolicy: structuredClone(input.cancellationPolicySnapshot) } } }
...
export async function updateAppointmentAction(formData: FormData) {
  ...
  const { data: updatedId, error: updateError } = await client.rpc('update_appointment_atomic', { p_appointment_id: appointmentId, p_person_id: personId, p_service_id: serviceId, p_starts_at: startsAt.toISOString(), p_ends_at: endsAt.toISOString(), p_next_status: nextStatus, p_cancellation_deadline_at: cancellationDeadlineAt }); if (updateError || updatedId !== appointmentId) throw new Error('AGENDA_UPDATE_FAILED')
  redirect('/agenda/gerenciar')
}
