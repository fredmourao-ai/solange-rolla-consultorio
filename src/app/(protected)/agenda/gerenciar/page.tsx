...
export default async function AgendaManagementPage() {
  ...
  return <>
    ...
    <section aria-labelledby="new-appointment-heading">
      <h2 id="new-appointment-heading">Nova consulta</h2>
      <form action={createAppointmentAction} className="stack-form">
        <label>Paciente
          <select name="person_id" required defaultValue="">
            <option value="" disabled>Selecione</option>
            {(people ?? []).map((person) => <option key={person.id} value={person.id}>{person.preferred_name || person.civil_name}</option>)}
          </select>
        </label>
        <label>Serviço
        ...
        <label>Data e hora
          <input name="starts_at_local" type="datetime-local" required />
        </label>
        <button type="submit">Criar consulta</button>
      </form>
    </section>

    <section aria-labelledby="edit-appointment-heading">
      <h2 id="edit-appointment-heading">Editar ou reagendar</h2>
      {(appointments ?? []).length === 0 ? <p>Nenhuma consulta futura encontrada.</p> : null}
      {(appointments ?? []).map((appointment) => <article key={appointment.id} className="card">
        <h3>{appointment.person?.preferred_name || appointment.person?.civil_name || 'Paciente'} — {appointment.service?.name || 'Consulta'}</h3>
        <p>Status: <strong>{appointment.status}</strong></p>
        <p>Prazo de cancelamento: {new Date(appointment.cancellation_deadline_at).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</p>
        <form action={updateAppointmentAction} className="stack-form">
          <input type="hidden" name="appointment_id" value={appointment.id} />
          <label>Paciente
            <select name="person_id" required defaultValue={appointment.person_id}>
              {(people ?? []).map((person) => <option key={person.id} value={person.id}>{person.preferred_name || person.civil_name}</option>)}
            </select>
          </label>
          <label>Serviço
            <select name="service_id" required defaultValue={appointment.service_id}>
              {(services ?? []).map((service) => <option key={service.id} value={service.id}>{service.name} ({service.duration_minutes} min)</option>)}
            </select>
          </label>
          <label>Data e hora
            <input name="starts_at_local" type="datetime-local" required defaultValue={formatSaoPauloDateTimeLocal(appointment.starts_at)} />
          </label>
