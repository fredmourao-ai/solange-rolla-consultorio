...
export async function issueMockNfseAction(formData:FormData){
 ...
 const [{data:profile},{data:treatment},{data:payer}]=await Promise.all([
 ...
 const {data:existing}=await client.from('fiscal_documents').select('id').eq('idempotency_key',idempotencyKey).maybeSingle(); if(existing){redirect('/fiscal/operacoes')}
 ...
 await client.from('fiscal_attempts').insert({fiscal_document_id:id,attempt_number:1,operation:'issue',status:'succeeded',provider_status:'issued',correlation_id:randomUUID(),finished_at:new Date().toISOString()})
...
export async function cancelMockNfseAction(formData:FormData){
 ...
 const {data:existing}=await client.from('fiscal_cancellation_events').select('id').eq('idempotency_key',key).maybeSingle(); if(!existing){const {error}=await client.from('fiscal_cancellation_events').insert({fiscal_document_id:id,idempotency_key:key,reason,requested_by:authorized.userId,provider_protocol:doc.protocol,status:'cancelled',completed_at:new Date().toISOString()}); if(error) throw new Error('FISCAL_CANCEL_EVENT_FAILED')}
 ...
 await client.from('fiscal_attempts').insert({fiscal_document_id:id,attempt_number:1,operation:'cancel',status:'succeeded',provider_status:'cancelled',correlation_id:randomUUID(),finished_at:new Date().toISOString()}).then(()=>undefined)
