...
import { buildAdministrativeExport, type AdministrativeExportRow } from '@/modules/reports/application/export-administrative'
...
export async function GET(request:NextRequest){
 ...
 const [payments,payables,events,fiscal]=await Promise.all([
  c.from('payments').select('paid_at,amount_cents,method').gte('paid_at',fromIso).lt('paid_at',toIso).order('paid_at'),
  c.from('payables').select('due_date,amount_cents,paid_cents,status,description').gte('due_date',start).lte('due_date',end).order('due_date'),
  c.from('events').select('starts_at,title,status,default_price_cents').gte('starts_at',fromIso).lt('starts_at',toIso).order('starts_at'),
  c.from('fiscal_documents').select('created_at,amount_cents,status,provider').gte('created_at',fromIso).lt('created_at',toIso).order('created_at'),
 ])
 ...
 const exported=await buildAdministrativeExport(format,rows,`${start} a ${end}`); const filename=`relatorio-administrativo-${start}-${end}.${exported.extension}`
 return new Response(exported.content,{headers:{'Content-Type':exported.mediaType,'Content-Disposition':`attachment; filename="${filename}"`,'Cache-Control':'private, no-store'}})
}
