...
-- cross-module-task: docs/task-contracts/appointments-default-cancellation-policy.json
...
select
  ...
  (
    select v.id
    from public.legal_document_versions as v
...
where d.key = 'cancellation_policy'
  and exists (
    select 1
    from public.legal_document_versions as available
    where available.document_id = d.id
      and available.version = 1
  )
  and not exists (
    select 1
