...
describe('repository governance contract', () => {
  ...
  it('prepares the local Supabase database for end-to-end tests', () => {
    ...
    expect(workflow).toContain('npx supabase@2.115.0 status -o env')
    ...
    expect(workflow).toContain('CLINICAL_ENCRYPTION_KEY_V1=')
    expect(workflow).toContain('CLINICAL_ENCRYPTION_ACTIVE_VERSION=1')
    expect(workflow).toContain('RATE_LIMIT_HMAC_KEY=')
    expect(workflow).toContain('PUBLIC_ACTION_HMAC_KEY=')
    expect(workflow).toContain('WHATSAPP_LIVE_ENABLED=false')
    expect(workflow).toContain('NFSE_LIVE_ENABLED=false')
    expect(workflow).toContain('DEMO_LOCAL_EMAIL=demo.owner@solange.invalid')
    expect(workflow).toContain('DEMO_LOCAL_PASSWORD=DemoLocalOnly!2026')
    expect(workflow).toContain('E2E_PORT=')
  })
