...
test('staff creates an appointment from the protected agenda management UI', async ({ page }) => {
  ...
  expect(sql(`select count(*) from public.appointments where person_id=${quote(personId)} and starts_at='2035-01-15T17:00:00Z'::timestamptz`)).toBe('1')
  expect(sql(`select count(*) from public.audit_events where action='appointment.created' and metadata->>'personId'=${quote(personId)}`)).toBe('1')
})
...
test('staff resolves a reschedule request while preserving the policy snapshot', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/agenda\/gerenciar$/)
  expect(sql(`select status||'|'||starts_at::text||'|'||policy_version::text from public.appointments where id=${quote(appointmentId)}`)).toContain('rescheduled|2035-01-20 18:00:00+00|1')
  expect(sql(`select count(*) from public.appointment_status_history where appointment_id=${quote(appointmentId)} and from_status='reschedule_requested' and to_status='rescheduled'`)).toBe('1')
})
