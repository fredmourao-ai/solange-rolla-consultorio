...
async function createFiscalPerson(page: Page, name: string, cpf: string) {
  ...
  await page.getByRole('button', { name: 'Cadastrar paciente' }).click(); await expect(page).toHaveURL(/\/pessoas$/)
...
async function createPastAppointmentAndCharge(page: Page, personId: string) {
  ...
  await page.goto(`/agenda?view=day&date=${slot.date}`, { waitUntil: 'domcontentloaded' }); const item = page.locator('li.appointment-calendar__item').filter({ has: page.locator(`input[value="${id}"]`) }); await item.getByText('Abrir consulta').click(); const status = item.getByRole('form', { name: 'Ações da consulta' }); await status.locator('select[name="command"]').selectOption('cancel_late'); await status.getByRole('button', { name: 'Aplicar' }).click()
  await expect.poll(() => sql(`select status from public.appointments where id=${q(id)}`)).toBe('cancelled_late'); await page.goto(`/agenda?view=day&date=${slot.date}`, { waitUntil: 'domcontentloaded' }); const charged = page.locator('li.appointment-calendar__item').filter({ hasText: nameFrom(personId) }); await charged.getByText('Abrir consulta').click(); await charged.getByRole('button', { name: 'Registrar cobrança' }).click()
...
test('owner elevates to AAL2 through the UI, writes a clinical record, and logs out', async ({ page }) => {
  ...
  const appointmentDate = sql(`select to_char(starts_at at time zone 'America/Sao_Paulo','YYYY-MM-DD') from public.appointments where id=${q(appointmentId)}`)
  ...
  await expect(page).toHaveURL(new RegExp(`/atendimentos/${appointmentId}$`))
  await page.getByRole('button', { name: 'Iniciar atendimento' }).click()
  await expect.poll(() => sql(`select status from public.appointments where id=${q(appointmentId)}`)).toBe('in_progress')
  await page.getByLabel('Evolução da sessão').fill('Registro clínico sintético de homologação, sem dado real.')
  await page.getByRole('button', { name: 'Finalizar atendimento e registrar evolução' }).click()
  await expect.poll(() => sql(`select count(*) from clinical.records where person_id=${q(personId)}`)).toBe('1')
  await expect.poll(() => sql(`select status from public.appointments where id=${q(appointmentId)}`)).toBe('completed')

  await page.goto(`/clinico/${personId}`, { waitUntil: 'domcontentloaded' })
  await expect(page.getByRole('heading', { name: 'Histórico médico / anamnese' })).toBeVisible()
  await page.getByLabel('Condições de saúde relevantes').fill('Condição sintética revisada')
  await page.getByLabel('Medicamentos em uso').fill('Medicação sintética 10 mg')
  await page.getByLabel('Alergias').fill('Alergia sintética')
  await page.getByLabel('Cirurgias e internações').fill('Cirurgia sintética em 2020')
  await page.getByLabel('Antecedentes familiares').fill('Antecedente sintético')
  await page.getByLabel('Hábitos e substâncias').fill('Sem tabagismo; rotina sintética')
  await page.getByLabel('Profissionais de saúde envolvidos').fill('Clínico sintético')
  await page.getByLabel('Alertas clínicos').fill('Alerta sintético de homologação')
  await page.getByLabel('Observações clínicas').fill('Autorreporte sintético revisado pela profissional.')
  await page.getByRole('button', { name: 'Registrar anamnese clínica' }).click()
  await expect.poll(() => sql(`select count(*) from clinical.medical_histories where person_id=${q(personId)}`)).toBe('1')
  expect(sql(`select position('Alergia sintética' in ciphertext) from clinical.medical_histories where person_id=${q(personId)} order by revision desc limit 1`)).toBe('0')
  expect(sql(`select position('Alergia sintética' in metadata::text) from public.audit_events where entity_type='medical_history' and metadata->>'personId'=${q(personId)} order by created_at desc limit 1`)).toBe('0')
  const clinicalSummary = page.getByLabel('Resumo clínico longitudinal')
  await expect(clinicalSummary.getByText('Alergia sintética', { exact: true })).toBeVisible()
  await expect(clinicalSummary.getByText('Medicação sintética 10 mg', { exact: true })).toBeVisible()
  await page.reload({ waitUntil: 'domcontentloaded' })
  await expect(page.getByLabel('Resumo clínico longitudinal').getByText('Alerta sintético de homologação', { exact: true })).toBeVisible()

  await page.getByLabel('Alergias').fill('Alergia sintética revisada')
  await page.getByRole('button', { name: 'Salvar nova revisão da anamnese' }).click()
  await expect.poll(() => sql(`select count(*) from clinical.medical_histories where person_id=${q(personId)}`)).toBe('2')
  await expect.poll(() => sql(`select max(revision) from clinical.medical_histories where person_id=${q(personId)}`)).toBe('2')
  await expect(page.getByLabel('Resumo clínico longitudinal').getByText('Alergia sintética revisada', { exact: true })).toBeVisible()
  await page.getByText('Ver histórico de revisões da anamnese').click()
  const historyPanel = page.getByLabel('Histórico médico / anamnese')
  await expect(historyPanel.getByText('Alergia sintética', { exact: true })).toBeVisible()
  await expect(historyPanel.getByText('Alergia sintética revisada', { exact: true }).last()).toBeVisible()
...
test('staff manages communication preferences and a distinct financial responsible through the UI', async ({ page }) => {
  ...
  await expect(page).toHaveURL(new RegExp(`/pessoas/${patientId}\\?status=updated$`))
  ...
  await expect(page).toHaveURL(new RegExp(`/pessoas/${patientId}/gerenciar\\?status=relationship_added$`))
