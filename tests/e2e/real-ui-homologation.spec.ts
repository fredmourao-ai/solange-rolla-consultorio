...
async function createPastAppointmentAndCharge(page: Page, personId: string) {
  ...
  await page.goto(`/agenda?view=day&date=${slot.date}`, { waitUntil: 'domcontentloaded' }); const item = page.locator('li.appointment-calendar__item').filter({ has: page.locator(`input[value="${id}"]`) }); await item.getByText('Abrir consulta').click(); const status = item.getByRole('form', { name: 'Alterar status da consulta' }); await status.locator('select[name="command"]').selectOption('cancel_late'); await status.getByRole('button', { name: 'Aplicar' }).click()
  await expect.poll(() => sql(`select status from public.appointments where id=${q(id)}`)).toBe('cancelled_late'); await page.goto(`/agenda?view=day&date=${slot.date}`, { waitUntil: 'domcontentloaded' }); const charged = page.locator('li.appointment-calendar__item').filter({ hasText: nameFrom(personId) }); await charged.getByText('Abrir consulta').click(); await charged.getByRole('button', { name: 'Cobrar falta/cancelamento fora do prazo' }).click()
