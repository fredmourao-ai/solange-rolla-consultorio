...
describe('E2E database command security', () => {
  it('keeps DB credentials out of argv and sanitizes subprocess failures', async () => {
    ...
    let commandArgs: string[] = []; let options: { env?: Record<string, string>; timeout?: number; killSignal?: string } = {}
    ...
    expect(options.env?.PGOPTIONS).toContain('statement_timeout=10000')
    expect(options.env?.PGOPTIONS).toContain('lock_timeout=5000')
