...
describe('resolveE2eRuntime', () => {
  ...
  it('selects the historical state audit suite only through an explicit allowlisted external suite', () => { const r = resolveE2eRuntime({ ...staging, E2E_EXTERNAL_SUITE: 'historical-state-audit' }); expect(r.projectTestMatch).toBe('**/agenda-historical-state-transitions.spec.ts') })
  it('selects the full published runtime parity suite only through an explicit allowlisted external suite', () => { const r = resolveE2eRuntime({ ...staging, E2E_EXTERNAL_SUITE: 'full-runtime-parity' }); expect(r.projectTestMatch).toBe('**/*.spec.ts') })
  it('rejects unknown external suites instead of widening the published UI test surface', () => { expect(() => resolveE2eRuntime({ ...staging, E2E_EXTERNAL_SUITE: 'anything' })).toThrow('E2E_EXTERNAL_SUITE_INVALID') })
